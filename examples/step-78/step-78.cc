...
namespace BlackScholesSolver{
    ...
    */
    ...
    double Solution<dim>::value(const Point<dim> &p, const unsigned int component) const
    {
        ...
        return_value += -std::pow(p(component),2) - std::pow(this->get_time(),2) + 6;
        return return_value;
    }
    ...
    /*
    The next piece is the declaration of the main class of this program. This is very similar to the Step-26 tutorial, with some modifications. 
    New matrices had to be added to calculate the A and B matrices, as well as the $V_{diff}$ vector mentioned in the introduction.
    */
    template <int dim>
    class BlackScholes{
    ...
    /*
    In the following classes and functions, we implement the various pieces of data that define this problem. 
    This includes the boundary values and the initial conditions that are used in this program and for which we need function objects. 
    The initial conditions and boundary values as described in the introduction.
    */
                                    ...
                                    const unsigned int component) const
    {
        ...
        return 2*(this->get_time()) - \
            std::pow(_sigma*p(component),2) - \
            2*_r*std::pow(p(component),2) - \
            _r*(-std::pow(p(component),2) - std::pow(this->get_time(),2) + 6);
    ...
    /*
    Now, we get to the implementation of the main class. This is the constructor, which sets the various parameters as described in the introduction.
    */
    template <int dim>
    ...
    template <int dim>
    RightBoundaryValues<dim>::RightBoundaryValues(double __s_price,
                                                    double __r)
    {
        _s_price = __s_price;
        _r = __r;
    ...
    /*
    The next function is the one that sets up the DoFHandler object, computes the constraints, and sets the linear algebra objects to their correct sizes. 
    We also compute the mass matrix here by calling a function from the library. We will compute the other 3 matrices next, because these need to be computed 'by hand'.
    */
    template <int dim>
    void BlackScholes<dim>::setup_system(){
        ...
        /*
        Below is the code to create the Laplace matrix with non constant coefficients. This corresponds to the matrix D in the introduction. 
        This non-constant coefficient is represented in the 'current_coefficient' variable.
        */
    ...
    */
    ...
    /*
    This is simply the function to build the solution together. For this, we create a new layer at each time, and then add the solution vector for that timestep. 
    The function then stitches this together with the old solutions using 'build_patches'.
    */
    template <int dim> 
    ...
    /*
    This is the function that refines the spatial mesh. This is exactly what was used in Step-26, so there is nothing new here.
    */
    template <int dim>
    ...
    /*
    This is somewhat unnecessary to have a function for the global refinement that we do. The reason for the function, is to 
    allow for the possibility of an adaptive refinement later.
    */
    ...
    /*
    Now we get to the main driver of the program. This is where we do all the work of looping through the timesteps and calculating the solution vector each time.
    Here at the top, we set the initial refinement value and then create a mesh. Then we refine this mesh once. Next, we set up the data_out_stack object to store our solution.
    Finally we create a label for where we should start when rerunning the first timestep and interpolate the initial condition onto the mesh.
    */
    template <int dim>
    ...
    for(unsigned int cycle=0;cycle<n_cycles;cycle++)
        ...
        // In the case of the line, the boundary id's are already what they should be

        

        /*
        Next, we run the main loop which runs until we exceed the maturity time. We first need to solve the non-advection terms first, so we do that here. 
        To do this, we first compute the right hand side of the equation, which is described in the introduction. Then we compute the left hand side of the equation
        that will need to be inverted to solve for $V_{diff}$. After all of this is done, we interpolate the boundary values and solve. This solution is stored into $V_{diff}$ 
        to be used in the next part of the Lie Splitting.
        */
        while(time < maturity_time){
            ...
            system_rhs.add((-1) * (1 - theta) * time_step * pow(sigma, 2) * 0.5, tmp);
            ...
            */
            ...
            // Now to compute the left side of the equation that needs to be inverted.
            system_matrix.copy_from(mass_matrix);
            system_matrix.add((theta) * time_step * pow(sigma, 2) * 0.5, laplace_matrix);
        ...
        // Convergence Table
        /*
        This next part is building the convergence table. First, we will create the headings and set up the cells 
        properly. During this, we will also prescribe the precision of our results. Then we will write the calculated errors
        based on the L2, H1, and Linfinity norms to the console and to the error LaTex file.
        */
